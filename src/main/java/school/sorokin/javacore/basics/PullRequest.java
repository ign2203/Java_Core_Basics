                        ...
                        for (int i = 0; i < contact; i++) {
                            if (names[i].equalsIgnoreCase(search_del)) { // создаем поиск требуемого контакта
                                for (int j = i; j < contact - 1; j++) { // мы создаем повторный цикл,
                                    // мы просто создаем цикл,
                                    // где условия работа с контактами должны на один меньше так как последний элемент мы будем удалять
                                    names[j] = names[j + 1];// здесь мо говорим, что все значение слева тебе равно значение справа до последнего элемента массива
                                    phoneNumbers[j] = phoneNumbers[j + 1];// с номерами аналогично
                                }
                                names[contact-1] = null;// как здесь я понял мы говорим что последний контакт мы обнуляем
                                phoneNumbers[contact-1] = null;// как здесь я понял мы говорим что последний контакт мы обнуляем
                                contact = contact -1;//
